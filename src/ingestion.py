...
def create_index(client: OpenSearch) -> None:
    """
    Creates the index in OpenSearch with the given configuration.
    ...
    if not index_config:
        logger.error("Failed to load index configuration.")
        return

    # Create the index if it doesn't exist
        client.indices.create(index=OPENSEARCH_INDEX, body=index_config)
...
def delete_index(client: OpenSearch) -> None:
    ...
    if client.indices.exists(index=OPENSEARCH_INDEX):
        ...
        logger.info(f"Delete index {OPENSEARCH_INDEX}: {response}")
    else:
...
def delete_document_by_name(document_name: str) -> Dict[str, Any]:
    """
